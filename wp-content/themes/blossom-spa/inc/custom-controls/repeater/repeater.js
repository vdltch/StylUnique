/*jshint -W065 */
var blossomspaRepeaterRow = function( rowIndex, container, label ){
	'use strict';
...
wp.customize.controlConstructor['blossom-spa-repeater'] = wp.customize.Control.extend({
	ready: function(){
		'use strict';
	...
	addRow: function( data ){
		...
		if ( template ) {
			...
			newRow = new blossomspaRepeaterRow(
				control.currentIndex,
    ...
    $(document).on( 'click', 'input[type="font"]', function(){
        ...
        if( ( ! $this.hasClass('ajax-running') ) && $this.siblings( '.font-awesome-list' ).length < 1 ){
            $.ajax({
				...
				url : ajaxurl,
				data: {
					action             : 'blossom_spa_get_fontawesome_ajax',
					blossom_spa_customize_nonce: blossom_spa_customize.nonce
				},
				beforeSend: function(){
